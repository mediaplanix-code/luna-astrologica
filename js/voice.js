// ============================================================
// VOICE.JS v2.0 — Verifica accesso prima di avviare sessione
// ============================================================

import { CONFIG } from './config.js';
import { getCurrentUser, getCurrentProfile } from './auth.js';
import { getSubscriptionStatus, hasActiveConsultPackage, useConsultPackage, getConsultPackageStatus } from './payments.js';
...
function handleUserVoiceInput(transcript) {
 ...
 speak(response, () => {
 // Dopo che ha parlato, riprende ad ascoltare
...
function updateVoiceUI(text, speaker) {
 ...
 if (speaker === 'user') {
 voiceText.innerHTML = `
 <div style="margin-bottom:0.75rem; padding:0.75rem; background:var(--bg-elevated); border-radius:0.5rem; border-left:3px solid var(--gold);">
 <div style="font-size:0.75rem; color:var(--text-dim); margin-bottom:0.25rem;">Tu</div>
 <div style="color:var(--text); font-size:0.9375rem;">${text}</div>
 </div>
 ` + voiceText.innerHTML;
 } else {
 voiceText.innerHTML = `
 <div style="margin-bottom:0.75rem; padding:0.75rem; background:rgba(245,158,11,0.08); border-radius:0.5rem; border-left:3px solid var(--gold);">
 <div style="font-size:0.75rem; color:var(--gold); margin-bottom:0.25rem; font-weight:600;">Luna</div>
 <div style="color:var(--text); font-size:0.9375rem; line-height:1.6;">${text}</div>
 </div>
 ` + voiceText.innerHTML;
 }
 }
...
export function startVoiceSession(category) {
 ...
 // Verifica accesso: abbonamento, regalo attivo, o pacchetto consulto
 const status = getSubscriptionStatus();
 const giftData = localStorage.getItem('luna_welcome_gift');
 let giftActive = false;
 if (giftData) {
 const gift = JSON.parse(giftData);
 if (gift.activated && Date.now() < gift.expiresAt) {
 giftActive = true;
 }
 }

 const consultStatus = getConsultPackageStatus();
 const hasConsult = consultStatus.hasPackage;
 ...
 // Se usa pacchetto consulto, marca come usato
 if (hasConsult && !status.active && !giftActive) {
 const pkg = consultStatus.packages[0];
 if (pkg) {
 voiceSession.usedPackageId = pkg.id;
 useConsultPackage(pkg.id);
 }
 }
