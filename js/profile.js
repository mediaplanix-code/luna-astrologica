...
// ===== HANDLER COMPATIBILITÀ INLINE (tendina) =====
export async function handleCompatInlineSubmit(e) {
    e.preventDefault();

    const name = document.getElementById('compatInlineName')?.value?.trim();
    const birthDateRaw = document.getElementById('compatInlineBirthDate')?.value;
    const birthTime = document.getElementById('compatInlineBirthTime')?.value;
    const city = document.getElementById('compatInlineBirthCity')?.value?.trim();
    const country = document.getElementById('compatInlineBirthCountry')?.value;
    ...
    try {
        const response = await fetch(`${CONFIG.WORKER_URL}/api/compatibility`, {
            ...
            body: JSON.stringify({
                user_id: userId,
                partner_name: name,
                partner_birthDate: birthDateRaw,
        ...
        renderCompatInlineResult(data, name);

    } catch (err) {
        console.error('Compatibilità inline error:', err);
        if (resultDiv) {
            resultDiv.innerHTML = `
                <div style="text-align:center; padding:1rem; color:var(--danger);">
