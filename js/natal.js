...
// FIX: Legge prima da natal_charts (Supabase), se manca chiama Render
// ============================================================
...
// ===== CARICA TEMA NATALE — CON CACHE DB =====
export async function loadNatalChart() {
  ...
  if (cachedChart) { console.log('📦 Cache memoria'); return cachedChart; }

  // 🌙 FIX: Prova prima a leggere da natal_charts (Supabase)
  const supabase = getSupabase();
  const userId = getCurrentUser()?.id;

  if (supabase && userId) {
    try {
      console.log('🔍 Cerco tema natale in natal_charts...');
      const { data: savedChart, error: dbErr } = await supabase
        .from('natal_charts')
        .select('*')
        .eq('user_id', userId)
        .single();

      if (!dbErr && savedChart && savedChart.planets && Object.keys(savedChart.planets).length > 0) {
        console.log('✅ Tema natale trovato in DB, uso quello');
        const chart = dbChartToResponse(savedChart);
        cachedChart = chart;
      ...
      if (dbErr && dbErr.code !== 'PGRST116') {
        console.warn('DB error natal_charts:', dbErr);
      } else {
        console.log('📝 Tema natale non trovato in DB, calcolo dal server...');
      }
    } catch (e) {
      console.warn('Errore lettura natal_charts:', e);
    }
  }

  // Calcolo dal server Render
  try {
    ...
    const res = await fetch(`${API_URL}/api/natal-chart`, {
      ...
      body: JSON.stringify({
        birthDate: utc.date, birthTime: utc.time,
        lat: profile.birth_latitude, lng: profile.birth_longitude,
        timezone: 'UTC',
        user_id: userId // 🌙 FIX: passa user_id per il salvataggio server-side
      })
    });
    if (!res.ok) throw new Error('Chart ' + res.status);
    cachedChart = await res.json();
    console.log('✅ Chart calcolato dal server:', cachedChart);
...
// 🌙 HELPER: Converte record DB in formato response
function dbChartToResponse(db) {
  const planets = db.planets || [];
  const houses = db.houses || [];
  const points = db.points || {};

  // Estrai moonSign dai points
  let moonSign = null;
  const moonPlanet = planets.find(p => p.key === 'moon');
  if (moonPlanet) moonSign = moonPlanet.sign;
  else if (points.moon_sign) moonSign = points.moon_sign;

  return {
    planets: planets,
    moonSign: moonSign,
    ascendant: points.ascendant || (houses[0] ? { ...houses[0] } : { name: '?', degree: 0 }),
    mc: points.mc || { name: '?', degree: 0 },
    houses: houses
  };
}
...
function drawWheelSVG(chart, container) {
  ...
  svgHTML += '<svg viewBox="0 0 ' + size + ' ' + size + '" style="width:100%;max-width:900px;height:auto;">';
  ...
  svgHTML += '<rect width="' + size + '" height="' + size + '" fill="#0a0a1a" rx="12"/>';

  // Cerchio esterno
  svgHTML += '<circle cx="' + cx + '" cy="' + cy + '" r="' + rOuter + '" fill="none" stroke="#2a2a4a" stroke-width="1.5"/>';
  ...
  for (let i = 0; i < 12; i++) {
    ...
    svgHTML += '<line x1="' + o.x + '" y1="' + o.y + '" x2="' + inn.x + '" y2="' + inn.y + '" stroke="' + color + '" stroke-width="1.2" opacity="0.6"/>';
  }

  // Cerchi interni
  svgHTML += '<circle cx="' + cx + '" cy="' + cy + '" r="' + rSign + '" fill="none" stroke="#3a3a5a" stroke-width="1"/>';
  svgHTML += '<circle cx="' + cx + '" cy="' + cy + '" r="' + rPlanet + '" fill="none" stroke="#3a3a5a" stroke-width="1" stroke-dasharray="4 4"/>';
  svgHTML += '<circle cx="' + cx + '" cy="' + cy + '" r="' + rCenter + '" fill="#151530" stroke="#4a4a6a" stroke-width="1.5"/>';
  ...
  for (let i = 0; i < 12; i++) {
    ...
    svgHTML += '<text x="' + n.x + '" y="' + n.y + '" text-anchor="middle" dominant-baseline="central" fill="#888" font-size="14" font-weight="bold">' + (i + 1) + '</text>';

    const s = polar((rSign + rOuter) / 2, mid);
    const sym = SIGN_SYMBOLS[SIGNS[signIdx]] || '?';
    svgHTML += '<text x="' + s.x + '" y="' + s.y + '" text-anchor="middle" dominant-baseline="central" fill="#b8a060" font-size="22">' + sym + '</text>';
  }
  ...
  (chart.planets || []).forEach((p, idx) => {
    ...
    svgHTML += '<circle cx="' + pt.x + '" cy="' + pt.y + '" r="14" fill="#0a0a1a" stroke="' + col + '" stroke-width="2" filter="url(#glow)"/>';
    svgHTML += '<text x="' + pt.x + '" y="' + pt.y + '" text-anchor="middle" dominant-baseline="central" fill="' + col + '" font-size="16">' + sym + '</text>';

    const dpt = polar(rPlanet + 40, lon);
    const deg = Math.floor(lon % 30);
    const min = Math.floor((lon % 30 - deg) * 60);
    svgHTML += '<text x="' + dpt.x + '" y="' + dpt.y + '" text-anchor="middle" dominant-baseline="central" fill="#aaa" font-size="10">' + deg + '°' + min + '\'';</text>';
  });
  ...
  aspects.forEach(asp => {
    ...
    if (p1 && p2) {
      ...
      svgHTML += '<line x1="' + pt1.x + '" y1="' + pt1.y + '" x2="' + pt2.x + '" y2="' + pt2.y + '" stroke="' + color + '" stroke-width="1" opacity="0.4" stroke-dasharray="3 3"/>';
    }
  });
  ...
  svgHTML += '<text x="' + cx + '" y="' + (cy - 12) + '" text-anchor="middle" fill="#FFD700" font-size="28" font-weight="bold">' + ascSym + '</text>';
  svgHTML += '<text x="' + cx + '" y="' + (cy + 8) + '" text-anchor="middle" fill="#b8a060" font-size="11">Ascendente</text>';
  svgHTML += '<text x="' + cx + '" y="' + (cy + 22) + '" text-anchor="middle" fill="#aaa" font-size="10">' + (asc?.name || '?') + ' ' + (asc?.degree !== undefined ? asc.degree + '°' : '') + '</text>';
  svgHTML += '<text x="' + cx + '" y="' + (cy + 36) + '" text-anchor="middle" fill="#888" font-size="9">MC: ' + (chart.mc?.name || '?') + ' ' + (chart.mc?.degree !== undefined ? chart.mc.degree + '°' : '') + '</text>';
